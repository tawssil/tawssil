...
const LiveDriverMap = dynamic(
  () => import("@/app/components/LiveDriverMap"),
  { ssr: false }
);
...
export default function OrderPage({
  ...
  async function loadDriverLocation(driverId: number) {
    const { data, error } = await supabaseBrowser
  ...
  useEffect(() => {
    ...
    if (orderChannelRef.current) {
      supabaseBrowser.removeChannel(orderChannelRef.current);
  ...
  useEffect(() => {
    ...
    if (locationChannelRef.current) {
      supabaseBrowser.removeChannel(locationChannelRef.current);
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              {driverLocation ? (
                ...
                <p className="mt-2 text-sm text-zinc-500">
                  Nog geen live locatie beschikbaar.
                </p>
              )}
            </div>
          </div>
